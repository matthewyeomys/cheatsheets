...
\begin{itemize}
    ...
    \item When methods implemented in child classes, have to be declared \verb|public|.
...
\subsection{Overriding Method that throws Exceptions}
When overriding a method that throws a checked exception, the overriding method must throw only the same or more specific checked exception than the overriden method. (LSP)

\subsection{The Error class}
Errors are for situations where the program should terminate as generally no way to recover. \verb|Exception| and \verb|Error| <: \verb|Throwable|.

\section{6. Generics}
Allow classes/methods to be defined without resorting to using the \verb|Object| type.\\
Generics are invariant in Java.

\subsection{Type Erasure}
At compile time, type parameters are replaced by \verb|Object| or the bounds (e.g. \verb|? extends Number| is replaced by \verb|Number|) \\
Given some class \verb|Pair|,
\begin{verbatim}
class Pair<S, T> {
...
\subsubsection{Overloading based on Type Arguments}
...
\subsubsection{Using Type Parameters in Static Contexts}
...
Generic array \textit{declaration} is fine but generic array \textit{instantiation} is not.
\begin{verbatim}
...
\deff{Bridge Methods} are compiler-generated synthetic methods inserted after type erasure to preserve polymorphism for generics.\\
Bridge erased method signature expected by the superclass/interface to the more specific method in the subclass. \\
...
\end{verbatim}
The compiler requires StringBox to implement a \verb|void get(Object)| method, hence it adds the following bridge method within \verb|StringBox|
\begin{verbatim}
void get(Object s) {
    this.get((String) s);
}
...
We cannot instantiate a generic type directly (e.g. \verb|new T[]|), hence we can get around this restriction as follows:
\begin{verbatim}
this.array = (T[]) new Object[size];
\end{verbatim}
This yields an \deff{unchecked warning}, a message from the compiler that it has done what it can abe because of type erasures, there could be an unpreventable runtime error. \\
\verb|@SuppressWarnings("unchecked")| cannot apply to an assignment but only to a declaration.\\
Using raw types can also lead to errors. Raw types only acceptable as an operand of \verb|instanceof|\\

\subsection{Wildcards} 

\begin{itemize}
    \item \textbf{Upper-Bounded:} \verb|? extends T|
    \item covariant (if S <: T, then \verb|A<? extends S> <: A<? extends T>|
    \item \textbf{Lower-Bounded:} \verb|? super T| 
    \item contravariant (if S <: T, then \verb|A<? super T> <: A<? super S>|
    \item \textbf{Unbounded:} \verb|?|
    \item \verb|Array<?>| is the supertype of all generic \verb|Array<T>|
\end{itemize}

\subsubsection{PECS Principle}
\textbf{PE} - if need to produce T values, use \verb|List<? extends T>|\\
\textbf{CS} - if need to consume T values, use \verb|List<? super T>|\\
if need both, use \verb|?|.
...
\end{verbatim}
\subsection{Rules for Type Inference}
\begin{itemize}
    \item \verb|Type1 <: T <: Type2| then \verb|T| is inferred as \verb|Type1|
    \item \verb|Type1 <: T| then \verb|T| is inferred as \verb|Type1|
    \item \verb|T <: Type2|, then \verb|T| is inferred as \verb|Type2| 
\end{itemize}
